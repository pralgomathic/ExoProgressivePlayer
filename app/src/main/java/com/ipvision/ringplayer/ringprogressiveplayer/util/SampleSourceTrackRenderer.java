 ...
 */
package com.ipvision.ringplayer.ringprogressiveplayer.util;

import android.util.Log;

import com.ipvision.ringplayer.ringprogressiveplayer.util.MediaCodecUtil.DecoderQueryException;
...
public abstract class SampleSourceTrackRenderer extends TrackRenderer {
  ...
  private long durationUs;
    private static final String TAG = "SSourceTrackRenderer";
  ...
  public SampleSourceTrackRenderer(SampleSource... sources) {
    this.sources = new SampleSourceReader[sources.length];
      Log.d(TAG,"Sources length: " + sources.length);
  ...
  protected boolean doPrepare(long positionUs) throws ExoPlaybackException {
    ...
    Log.d(TAG,"Sources length:(doPrepare) " + sources.length);
    for (int i = 0; i < sources.length; i++) {
    ...
    Log.d(TAG,"Total Source Track: " + totalSourceTrackCount);
    long durationUs = 0;
